...
def _ensure_datetime_series(df: pd.DataFrame, col: str) -> pd.Series:
    ...
    This keeps Pylance happy for .dt operations.
    """
...
def apply_hours_of_operation(df: pd.DataFrame, hoo: HoursOfOperation) -> pd.DataFrame:
    ...
    assert is_datetime64_any_dtype(s)
...
def allocate_volume_to_intervals(
    ...
    *,
    ...
    if volume_rounding == "round":
        # require integer-ish daily_volume to preserve totals as counts
