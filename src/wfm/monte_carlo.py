...
VolumeDist = Literal["poisson", "normal", "lognormal"]
AHTDist = Literal["normal", "lognormal"]

# Production safety cap (protects your Azure instance)
MAX_SIMS_DEFAULT = 100
...
def _simulate_interval(
    *,
    ...
    # Closed or no volume => stable zero outputs
    ...
    # Apply engine-side cap for safety
    n_sims = int(cfg.n_sims)
    if n_sims <= 0:
        raise ValueError("cfg.n_sims must be > 0")
    n_sims = min(n_sims, MAX_SIMS_DEFAULT)
    ...
    out["sla_breach_rate"] = float(np.nanmean(breach)) if not np.all(np.isnan(breach)) else np.nan
    return out
...
def run_interval_monte_carlo(
    *,
    ...
    """
    Runs Monte Carlo per interval and returns interval_df joined with simulation metrics.

    interval_df must include:
      interval_start, interval_minutes, volume, aht_seconds, is_open

    Output columns appended:
      scheduled_mean/p50/p90/p95, on_phone_mean/p50/p90/p95, sla_breach_rate, asa_mean, occ_mean
    """
    ...
    # One RNG for the whole run (fast + consistent)
    rng = np.random.default_rng(int(cfg.seed))

    rows: list[Dict[str, Any]] = []
    for _, r in df.iterrows():
        metrics = _simulate_interval(
...
__all__ = ["MonteCarloConfig", "run_interval_monte_carlo", "validate_interval_df", "MAX_SIMS_DEFAULT"]